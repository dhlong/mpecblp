...
for reps=1:starts,
    ...
    Opt = opti(...
        'fun',@(x)GMMMPEC_f(x),...
        'grad',@(x)GMMMPEC_grad_f(x),...
        'nl', @(x)GMMMPEC_c(x),cl,cu,...
        'jac', @(x)GMMMPEC_grad_c(x),...
        'jacstr',@() sparse(ConsPattern),...
        'hess',@(x,sigma,lambda) GMMMPEC_hess(x,sigma,lambda),...
        'hstr',@() sparse(HessianPattern),...
        'bounds',x_L,x_U,...
        'x0',x0,...
        'options',opts);
    ...
    [X,fval_rep,exitflag,info] = solve(Opt);    
